...
class DualSub(_PluginBase):
    ...
    _run_now = False
    _path_list = ""
    _transfer_paths = ""  # 入库自动执行的目录白名单, 每行一个路径前缀, 留空=全部生效
    ...
    def init_plugin(self, config: dict = None):
        ...
        self._transfer_paths = config.get("transfer_paths", "")
        ...
        if self._clear_history:
            ...
            self.update_config({**self._build_config(), "clear_history": False})
    ...
    @staticmethod
    def _track_looks_mixed(title: str) -> bool:
        """根据字幕轨 title 判断是否可能是中英混合轨"""
        if not title:
            return False
        t = title.lower()
        keys = ("双语", "中英", "中英双语", "dual", "bilingual", "chs&eng",
                "chs&eng", "chs.ass", "chs&eng", "中字英音", "中英字幕")
        return any(k in t for k in keys) or ("英" in t and "中" in t)
    ...
    def generate_dual(self, video_path: str, zh_idx: int, en_idx: int,
        ...
        支持两种来源:
        - 双轨: zh_idx / en_idx 都有值, 分别提取中文轨、英文轨再合并
        - 单轨混合: en_idx 为 None 时, 若 zh 轨内容含中英混排行, 用 split_mixed 拆分后合并
        合并时每条字幕最多 2 行(英 1 + 中 1), 避免多行挡画面。
        """
        logs: List[str] = []
        ...
        try:
            ...
            if en_idx is not None:
                ...
                split_zh, split_en = split_mixed(zh_items)
                if not split_en:
                    # 整条轨都是纯中文, 没有英文可配对
                    return False, "字幕轨为纯中文, 无英文行", logs
    ...
    def process_video(self, video_path: str) -> Tuple[str, str]:
        ...
        with self._file_lock(video_path):
            ...
            if (zh is None or en is None) and not (zh is None and en is not None and self._ai_enabled):
                ...
                if "tracks" in probe:
                    for t in probe["tracks"]:
                        if is_chinese(t["lang"]) and self._track_looks_mixed(t.get("title", "")):
                            mixed_idx = t["index"]
                            break
                if mixed_idx is None:
                    return TaskStatus.IGNORED.value, "缺少中文或英文字幕轨"
                # 单轨中英混合: en_idx 传 None, generate_dual 会用 split_mixed 拆分
                zh, en = mixed_idx, None
    ...
    def on_transfer_complete(self, event: MPEvent):
        ...
        try:
            ...
            # 入库目录白名单过滤
            allow_paths = self._parse_allow_paths()
            if allow_paths:
                # 只处理路径前缀匹配白名单的文件
                filtered = []
                for fp in file_list:
                    norm = str(fp).replace("\\", "/").lower()
                    if any(norm.startswith(p) for p in allow_paths):
                        filtered.append(fp)
                skipped = len(file_list) - len(filtered)
                if skipped > 0:
                    logger.info(f"[DualSub] 入库过滤: {title or '(未知)'} 跳过 {skipped} 个不在白名单目录的文件")
                file_list = filtered
                if not file_list:
                    logger.info(f"[DualSub] 入库 {title or '(未知)'} 全部文件不在白名单目录, 跳过")
                    return
    ...
    def _parse_allow_paths(self) -> list:
        """解析入库白名单目录, 返回小写、正斜杠结尾的路径前缀列表"""
        if not self._transfer_paths or not self._transfer_paths.strip():
            return []
        result = []
        for line in self._transfer_paths.splitlines():
            p = line.strip()
            if not p:
                continue
            # 统一路径格式: 小写, 正斜杠, 去尾部多余斜杠
            p = p.replace("\\", "/").rstrip("/")
            if p:
    ...
    def add_task(self, video_file: str, source: str) -> bool:
        ...
        if self._tasks:
            ...
            for tid, t in self._tasks.items():
                if t.get("video_file") != video_file:
                    continue
                status = t.get("status")
                if status in (TaskStatus.IN_PROGRESS.value, TaskStatus.PENDING.value):
                    # 正在处理或排队中, 不重复加
                    logger.info(f"[DualSub] 任务进行中, 跳过: {video_file}")
                    return False
                if status == TaskStatus.COMPLETED.value:
                    ...
                    if has_product:
                        ...
                        return False
                    # 产物被删除 -> 标记旧记录为失效, 允许重新入队
                    logger.info(f"[DualSub] 产物已被删除, 重新入队: {video_file}")
                ...
                try:
                    ...
                    if p.is_dir():
                        result["dirs"].append({
                            "name": name,
                            "path": str(p),
                            "poster": self._find_dir_poster(p),
                        })
    ...
    def _probe_video_item(self, p: Path) -> dict:
        ...
        if zh is None or en is None:
            probe = self.probe_subtitles(str(p))
            if "tracks" in probe:
                for t in probe["tracks"]:
                    if is_chinese(t["lang"]) and self._track_looks_mixed(t.get("title", "")):
                        mixed = True
        ...
        return {
            ...
            "zh": zh is not None or mixed,
            "en": en is not None or mixed,
            "has_dual": has_dual,
            "status": status,
            "poster": self._find_episode_thumb(p),
        }

    @staticmethod
    def _find_episode_thumb(video_path: Path) -> Optional[str]:
        """找单集缩略图: 同名 .jpg/.thumb.jpg, 没有则返回 None"""
        stem = video_path.stem
        d = video_path.parent
        for candidate in [d / f"{stem}.jpg", d / f"{stem}-thumb.jpg", d / f"{stem}.thumb.jpg"]:
            if candidate.exists():
                return str(candidate)
        return None

    @staticmethod
    def _find_dir_poster(dir_path: Path) -> Optional[str]:
        """找目录封面图: poster.jpg > seasonXX-poster.jpg > fanart.jpg > 同名.jpg"""
        d = dir_path
        # poster.jpg
        p = d / "poster.jpg"
        if p.exists():
            return str(p)
        # seasonXX-poster.jpg (Season N 目录)
        name = d.name.lower()
        if name.startswith("season"):
            sn = name.replace("season", "").strip()
            for cand in [d / f"season{sn}-poster.jpg", d / f"season{sn.zfill(2)}-poster.jpg"]:
                if cand.exists():
                    return str(cand)
        # 目录同名 .jpg (电影库常见)
        p2 = d / f"{d.name}.jpg"
    ...
    def api_regenerate(self, path: str = ""):
        ...
        try:
            ...
            # 删除所有旧字幕产物
            deleted = []
            for suffix in (self._subtitle_suffix, ".zh-CN.ass", ".zh-CN.srt", ".dual.srt"):
                old = video.with_name(video.stem + suffix)
                if old.exists():
                    old.unlink()
    ...
    async def api_poster(self, path: str = ""):
        """返回媒体封面图 (StreamingResponse, 供前端 VImg 加载)"""
        img_path = unquote(path or "").strip()
        if not img_path:
            return {"success": False, "message": "缺少 path 参数"}
        # 安全: 必须是图片文件, 且路径必须在 browse_root 之下
        safe = self._safe_path(img_path)
        if not safe:
            return {"success": False, "message": "路径无效或越界"}
        p = Path(safe)
        if not p.is_file():
            return {"success": False, "message": "文件不存在"}
        if p.suffix.lower() not in (".jpg", ".jpeg", ".png", ".webp", ".gif"):
            return {"success": False, "message": "非图片文件"}
        ...
        async def file_stream():
            try:
                async with aiofiles.open(str(p), mode="rb") as f:
                    while chunk := await f.read(8192):
                        yield chunk
            except Exception as e:
                logger.error(f"[DualSub] 读取封面图失败 {p}: {e}")
                                ...
                                'content': [
                                    {
                                        ...
                                        'props': {
                                            ...
                                            'label': '入库生效目录',
                                            'rows': 3,
                                            'placeholder': '容器内绝对路径, 每行一个\n如:\n/vol2/1000/Emby观影库/欧美剧\n/vol2/1000/Emby观影库/日韩剧\n留空 = 所有入库都生效',
                                            'hint': '仅这些目录(含子目录)下入库的视频才会自动生成双语字幕。留空则所有入库都生效'
                                        }
                                    }
                                ]
                            }
    ...
    def _build_browse_tab(self) -> dict:
        ...
        for d in dirs:
            rows.append({
                "component": "div",
                "props": {
                    "class": "d-flex align-center pa-4 rounded-lg mb-2",
                    "style": "background: #3a3a4a; cursor: pointer; min-height: 56px;",
                },
                "events": {
                    "click": {"api": f"plugin/DualSub/browse?path={quote(d['path'])}", "method": "get"}
                },
                "content": [
                    {"component": "VIcon", "props": {"icon": "mdi-folder", "color": "amber", "class": "me-4", "size": "x-large"}},
                    {"component": "span", "props": {"class": "text-body-1 font-weight-bold text-truncate flex-grow-1", "style": "font-size: 16px; color: #ffffff;"}, "text": d["name"]},
                    {"component": "VIcon", "props": {"icon": "mdi-chevron-right", "color": "#cccccc", "class": "ms-2", "size": "large"}},
                ],
            })
        ...
        for v in videos:
            if v["status"] == "processing":
                ...
                badge = {"text": "缺中/英轨", "color": "error", "icon": "mdi-alert-circle-outline"}
                can_process = False
            else:
                badge = {"text": "可处理", "color": "primary", "icon": "mdi-subtitles-outline"}
                can_process = True

            right_content = [
                {"component": "span", "props": {"class": "text-body-2 me-4", "style": "font-size: 14px; white-space: nowrap; color: #aaaaaa;"}, "text": v["size_text"]},
                {
                    "component": "VChip",
                    "props": {"size": "small", "color": badge["color"], "variant": "flat",
                              "prepend-icon": badge["icon"], "class": "me-3"},
                    "text": badge["text"],
                },
            ]
            if can_process:
                right_content.append({
                    "component": "VBtn",
                    "props": {"color": badge["color"], "variant": "flat", "size": "default",
                ...
                right_content.append({
                    ...
                    "props": {"color": "warning", "variant": "tonal", "size": "small",
                              "prepend-icon": "mdi-refresh", "class": "me-2"},
                    "text": "重新生成",
                    "events": {
            ...
            rows.append({
                ...
                "props": {
                    "class": "d-flex align-center pa-4 rounded-lg mb-2",
                    "style": "background: #2a2a36; border: 1px solid #444455; min-height: 56px;",
                },
                "content": [
                    {"component": "VIcon", "props": {"icon": "mdi-file-video-outline", "color": "indigo-lighten-2", "class": "me-4", "size": "large"}},
                    {"component": "span", "props": {"class": "text-body-1 text-truncate flex-grow-1", "style": "font-size: 16px; color: #ffffff;"}, "text": v["name"]},
                ] + right_content,
            })
